...
grid = np.ones((SIZE, SIZE), dtype=int)  # tudo começa como terra
...
for x in range(0, 100, 4):      # espaçamento horizontal
    for y in range(0, 50, 4):  # espaçamento vertical
...
def criar_individuo(especie):
    ...
    varA = random.randint(0, SIZE - 1) # Changed to use randint for integer coordinates
    varB = random.randint(0, SIZE - 1) # Changed to use randint for integer coordinates
...
def mutacao(ind):
    ...
    val = random.randint(3, 4)  # Index 3 is x, 4 is y
    var = random.uniform(-5, 5)
    ind[val] = int(max(0, min(SIZE - 1, ind[val] + var))) # Ensure within bounds and integer


    mutation_rate_movements = 0.05 # 5% chance to mutate each move
...
def evolucao(pai1, pai2,especie):
    ...
    for i in range(N_MOVES): # Child must have N_MOVES
    ...
    filho = [
        ...
        c2,
        c3,
        int((pai1[3] + pai2[3]) / 2), # Cast averaged coordinates to int
        int((pai1[4] + pai2[4]) / 2), # Cast averaged coordinates to int
        movimentos_filho.copy(), # current_movs is a copy of the original for consumption
        0,
        movimentos_filho.copy() # original_movs for future evolution - ensure it's a copy
    ]
...
def genocidio(lista, melhor_idx,especie):
    nova = [lista[melhor_idx]] # Corrected: use lista[melhor_idx] to get the actual animal
...
def select_parent_tournament(lista, tournament_size=3):
    ...
    if len(lista) < tournament_size:
        # If not enough individuals for a full tournament, just pick randomly
...
def darwin(lista, especie):
    ...
    for _ in range(len(lista) - 1):
        ...
        # Ensure parents are not None (if list was empty for some reason)
        if parent1 is not None and parent2 is not None:
            nova_lista.append(evolucao(parent1, parent2, especie))
        else:
            # Fallback if selection fails (e.g., list became empty unexpectedly)
...
valores_finais = []
def clamp_pos(x, y):
    x = int(max(0, min(SIZE-1, x))) # Ensure return values are integers
    y = int(max(0, min(SIZE-1, y))) # Ensure return values are integers
    return x, y
random.seed(42)
np.random.seed(42)
...
for p in range(50):
  for r in range(N_MOVES):
      ...
      plot_grid = original_terrain_layout.copy() # Start with clean terrain
      ...
      plt.clf()
      plt.imshow(plot_grid, origin="lower", cmap=cmap, vmin=0, vmax=8)
      plt.title(f"Rodada {r+1} — vivos: P={contar_vivos(peixes)} H={contar_vivos(herbivoros)} "
  ...
  valores = [peixes[melhor_todos(peixes)][5], herbivoros[melhor_todos(herbivoros)][5], mam_peixes[melhor_todos(mam_peixes)][5], mam_herb[melhor_todos(mam_herb)][5], mosquitos[melhor_todos(mosquitos)][5]]
